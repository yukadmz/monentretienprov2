                ...
                <div class="card-dashboard">
                    <a href='modif_candidature.php?id=<?php echo $entretien['id']; ?>'>
                        ...
                        if (estCandidaturePasseeAncienne($entretien)) {
                            echo '<i class="fas fa-clock clock"></i>';
                            // Calcul de la durée entre la date d'entretien et la date actuelle
                            $interviewDate = new DateTime($entretien['date_entretien']);
                            $currentDate = new DateTime();
                            $diff = $currentDate->diff($interviewDate);
                            // Affichage de la durée
                            echo $diff->format('  %a jours | <br>');
                        }
                        echo $entretien['entreprise'] . ' - ' . $entretien['poste'] . ' - ' . date("d/m/Y", strtotime($entretien['date_entretien'])); 
                        ?></h3>
                    </a>
                </div>
        <?php
